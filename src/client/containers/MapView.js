...
export default class MapView extends Component {
  ...
  addTileLayer() {
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {}).addTo(this.refs.map.leafletElement);
  }
  buildCrumbs() {
    let crumbList = '';
  ...
  infoControl() {
    ...
    info.update = function(props) {
      this._div.innerHTML =
        '<h4>US Flu Data</h4>' +
        (props ? '<b>' + props.name + '</b><br />' + props.density + ' flu cases' : 'Hover over a state');
    };
